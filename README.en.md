...
**v3.7 — Auto-PDF (font-subsetted <500KB) + Fabrication Gate + Stress-Tested Pipeline**
...
- **PDF**: Auto-generated by the Agent via `scripts/gen_resume_pdf.py` (Playwright headless render + PyMuPDF font subsetting) → A4, single-page, <500KB. Browser Ctrl+P is an optional fallback.
...
Filled HTML (Swiss International Style)
  ...
  Browser Ctrl+P → Save as PDF  (or auto-generated subsetted PDF via scripts/gen_resume_pdf.py)
```
...
- **No PDF/DOCX dependencies for core rendering** — Browser handles PDF natively, OR use `scripts/gen_resume_pdf.py` (optional `playwright` + `pymupdf`) for a font-subsetted <500KB PDF. DOCX dropped entirely.
...
│   ├── gen_resume_pdf.py                 # HTML → PDF (Playwright + PyMuPDF, font-subsetted <500KB)
...
- HTML is the primary output format. PDF is auto-generated (font-subsetted <500KB) via `scripts/gen_resume_pdf.py`, or manually via browser Ctrl+P.
...
|---------|------|------------|
| **v3.7** | 2026-07 | Auto-PDF pipeline (font-subsetted <500KB via Playwright+PyMuPDF), A8/A9 anti-patterns, version bump |
